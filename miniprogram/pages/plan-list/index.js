...
Page({
  ...
  onShow() {
    ...
    if (t) {
      app.globalData.uploadToast = null;
      wx.showToast({ title: t.msg, icon: t.isSuccess ? 'success' : 'none' });
    }
  ...
  // 主包页面拿不到 threejs-miniprogram (仅打包到 cabinet 分包),所以这里只做
  // "文件名合法性校验 + 数据交接",实际上传/解析/入库交给分包内的 upload-processor 页面。
  onConfirmUploadModal(e) {
    ...
    this.setData({ uploadModalVisible: false });
    wx.navigateTo({ url: '/cabinet/pages/upload-processor/index' });
  },
